 ...
 *
 * NOTES:
 * - Idempotent: will not create a second coupon for an entry if one already exists; stores the code in entry meta
 *   under created_coupon_code.
        ...
        function () {
            ...
            class BLD_CouponCodeWorkflowStep extends Gravity_Flow_Step {
                ...
                public function replace_coupon_code_merge_tag( $text, $form, $entry, $url_encode = false, $esc_html = true ) {
                    ...
                    $coupon_code = 0 !== $eid ? gform_get_meta( $eid, 'created_coupon_code' ) : '';
                    if ( '' === $coupon_code ) {
                ...
                public function process() {
                    ...
                    // Idempotency: if a coupon was already created for this entry, do not create another.
                    $eid         = isset( $entry['id'] ) ? (int) $entry['id'] : 0;
                    $existing_cc = 0 !== $eid ? gform_get_meta( $eid, 'created_coupon_code' ) : '';
                    if ( '' !== $existing_cc ) {
                        $this->add_note( sprintf( 'Coupon already created for this entry: %s', $existing_cc ) );
                        return true;
                    }
                    ...
                    gform_update_meta( $eid, 'created_coupon_code', $final_code );
